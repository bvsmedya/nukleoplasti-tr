          ...
          {treatments.map((item) => (
            <Link key={item.id} href={item.link} className="group relative h-96 rounded-2xl overflow-hidden shadow-xl block">
              ...
              <div className="absolute bottom-0 left-0 p-8 w-full z-10">
                ...
                <p className="text-gray-200 mb-4 opacity-0 group-hover:opacity-100 transition-opacity font-medium">{item.desc}</p>
                <span className="inline-block bg-blue-600 text-white px-6 py-2 rounded-full text-sm font-bold">İncele</span>
              </div>
            </Link>
          ))}
        </div>
